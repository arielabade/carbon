#BILSTM60EPOCHSBASELINEEVALUATION
#Imports
import pandas as pd
import numpy as np
...
# File paths, including NEB in training
file_paths_train = [
...
# Building the RNN model with Bi-LSTM
vocab_size = len(tokenizer.word_index) + 1
embedding_dim = 64

model = Sequential([
    Embedding(input_dim=vocab_size, output_dim=embedding_dim, input_length=max_len),
    Bidirectional(LSTM(64, return_sequences=True)),  # First Bi-LSTM layer with more units
    Dropout(0.2),
    Bidirectional(LSTM(64, return_sequences=True)),  # Second Bi-LSTM layer to capture more patterns
    Dropout(0.2),
    Bidirectional(LSTM(32)),  # Third Bi-LSTM layer to refine learned patterns
    Dropout(0.2),
    Dense(128, activation='relu'),  # Larger Dense layer for greater learning capacity
    Dropout(0.2),
...
# Function to calculate evaluation metrics with 4 decimal precision
