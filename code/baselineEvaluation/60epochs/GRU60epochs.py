...
# Splitting data into training (80%), validation (10%), and test (10%)
...
model = Sequential([
    ...
    GRU(32, return_sequences=True),  # First GRU layer
    Dropout(0.2),
...
# Function to calculate evaluation metrics with 4-decimal precision and normalization from 0 to 1
def evaluate_model(generator, y_true):
    # Generates predictions
    predictions = model.predict(generator)
    predictions = (predictions > 0.5).astype(int).flatten()  # Converts to 1D array

    # Displays data length in predictions and y_true
    ...
    # Returns metrics with 4-decimal precision
